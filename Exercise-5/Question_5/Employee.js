...
function filterSalaryGreaterThan5000() {
  outputList = Emp_list.filter(elem => elem.salary > 5000);
  display(outputList);
}
...
function filterSalaryLessThan1000AndAgeGreaterThan20() {
  ...
  var counter = 0;
  outputList = [];
  for (elem in Emp_list){
    if(Emp_list[counter].salary < 1000 && Emp_list[counter].age > 20) {
      Emp_list[counter].salary *= 5;
      outputList.push(Emp_list[counter]);
    }
    counter++;
  }
  display(outputList);
...
function groupByAge() {
  ...
  for(i=10; i<100; i+=10) {
    groupDisplay(Emp_list.filter(elem => elem.age > i && elem.age <= (i+10)));
  }
}
